...
@section('content_service')
    ...
    @isset($fudsData)
        @empty($fudsData)
            <p>We are soory ! ,no item right now</p>
        @else
            @foreach ($fudsData as $item)
                <div class="cntMenu">
                    <div class="cntListFood">
                        <section id="burger-card">
                            @foreach ($item as $i)
                                        ...
                                        @if (session('isBuilding') == true)
                                            ...
                                            @else
                                                <button class="btnOrder addTOMeal" id="btnAddToMeal"
                                                    ...
                                                    <svg xmlns="http://www.w3.org/2000/svg" class="iconsNotdIn"
            ...
            <script>
                ...
                let ListsFoodsEm = document.querySelectorAll(".cntMenu");
                ListsFoodsEm.forEach(elm => effectOnListType(elm.querySelector("section"), elm))

                function effectOnListType(listElement, fatherElement) {
                    let WindowWifth = window.innerWidth;
                    if (WindowWifth > 1500) {
                        WindowWifth = 1500
                    }
                    D([fatherElement], "FatherOfScrollButton");
                    let elemWidth = listElement.offsetWidth;
                    let scrollPx = listElement.children[0].offsetWidth;
                    let conter = 0;
                    let toRight = true
                    let elemInter;
                    let isParentWidthGreatThanWindows = elemWidth > WindowWifth;
                    // if (isParentWidthGreatThanWindows) {

                    function startInterval() {
                        if (elemInter == undefined) {
                            fatherElement.removeEventListener('mouseover', startInterval)
                            listElement.style.transition = ' 10s linear';
                            elemInter = setInterval(() => {
                                if (conter < elemWidth - WindowWifth && toRight) {
                                    conter += scrollPx / 2;
                                } else {
                                    toRight = false
                                    if (conter == 0) {
                                ...
                                listElement.style.transform = `translateX(${-conter}px)`
                            }, 10000);
                        }
                    ...
                    let isMouseIn = false
                    fatherElement.addEventListener('mouseover', () => {
                        isMouseIn = true
                    })
                    ...
                    let clickedByOneOfButtons = false;

                    buttonLeft.onclick = () => {
                        if (conter > 0) {
                            listElement.style.transition = '.2s'
                            conter -= scrollPx;
                            listElement.style.transform = `translateX(${-conter}px)`
                        }
                    ...
                    buttonRight.onclick = () => {
                        if (conter < elemWidth - WindowWifth) {
                            listElement.style.transition = '.2s'
                            conter += scrollPx;
                            listElement.style.transform = `translateX(${-conter}px)`
                    ...
                    fatherElement.onclick = () => {
                        if (!clickedByOneOfButtons) {
                            clickedByOneOfButtons = true
                            clearInterval(elemInter);
                            listElement.style.transform = window.getComputedStyle(listElement).transform
                            elemInter = undefined;

                            function handelMouseOut2() {
                                isMouseIn = false;
                                setTimeout(() => {
                                    if (!isMouseIn) {
                                        startInterval();
                                        isMouseIn = true
                                        clickedByOneOfButtons = false;
                                        fatherElement.removeEventListener('mouseout', handelMouseOut2);
                                    }
                                }, 1000);
                            }
                            fatherElement.addEventListener('mouseout', handelMouseOut2)
                        }

                    }

                    buttonLeft.className = 'button_scroll_Left displayFlex';
                    buttonLeft.innerHTML = `
                            <svg xmlns="http://www.w3.org/2000/svg" class="icon icon-tabler icon-tabler-arrow-big-left-lines-filled"  viewBox="0 0 24 24" >
                    ...
                    buttonRight.className = 'button_scroll_right displayFlex';
                    buttonRight.innerHTML = `
                            <svg xmlns="http://www.w3.org/2000/svg" class="icon icon-tabler icon-tabler-arrow-big-right-lines-filled" viewBox="0 0 24 24">
                            <path stroke="none" d="M0 0h24v24H0z" fill="none"/>
                    ...
                    fatherElement.appendChild(buttonLeft);
                    fatherElement.appendChild(buttonRight);
